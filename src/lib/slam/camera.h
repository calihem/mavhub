 ...
 * \param[in] rt Rotation + translation vector
 * \param[out] jac Jacobian of rotation and translation \f$ \frac{\partial f}{\partial qr \partial r}\f$.
 */
template<typename T>
 ...
 * \param[in] objectpoint 3D object point
 * \param[in] qt rotation quaternion vector part + translation vector
 * \param[out] jac_qt Jacobian of rotation and translation \f$ \frac{\partial f}{\partial qr \partial r}\f$.
 * \param[out] jac_s Jacobian of structure \f$ \frac{\partial f}{\partial opject\_point}\f$.
 */
template<typename T>
void ideal_pinhole_model_quatvec_jac(const T objectpoint[3],
		const T qt[6],
 ...
 * \brief Inverse camera model to use with euler angles or quatvec.
 * 
 * This is only a partial template Specialization for convenience.
 */
template<typename T, void(*R)(const T[3], T[9])>
void inverse_ideal_pinhole_model(const T *idealpoints,
		const T rt[6],
...
/**
 * \brief Calculates the projection of a 3D point to a 2D image point using quaternions.
 * \tparam T Base type of points, e.g. float or double
 * \param[in] objectpoints Pointer to an array of 3D object points
...
/**
 * \brief Calculates the jacobians of the pinhole projection.
 * \param[in] objectpoint 3D object point
...
void ideal_pinhole_model_euler_jac(const T objectpoints[3],
	...
	jac[0][0] = b2*inv_y3 - b3*y1*inv_y3_square;
	// partial v / partial phi
	jac[1][0] = -(b1*inv_y3 + b3*y2*inv_y3_square);
	// partial u / partial theta
	jac[0][1] = b4*inv_y3 + b5*y1*inv_y3_square;
	// partial v / partial theta
	jac[1][1] = b6*inv_y3 + b5*y2*inv_y3_square;
	// partial u / partial psi
	jac[0][2] = (t2-y2)*inv_y3;
...
void ideal_pinhole_model_quatvec_jac(const T objectpoint[3],
	...
	// partial u / partial q2
	jac[0][1] = (p1*q1+t175*t65+t180*q0-t185*q3+t192*q2+t93)*t127-t147*t216;
	// partial v / partial q2
	jac[1][1] = t206*t127-t159*t216;
	const T t240 = -q3/q0;
	...
	// partial u / partial q3
	jac[0][2] = (p2*q1+t240*t65+q0*t245-t250*q3-t81+t257*q2)*t127-t147*t281;
...
void ideal_pinhole_model_quatvec_jac(const T objectpoint[3],
	...
	const T t159 = t141/t145;
	// partial v / partial q1
	jac_qt[1][0] = t107*t127-t159*t118;
	const T t175 = -q2/q0;
	const T t180 = t175*p0+p2;
	const T t185 = t175*p1;
	const T t192 = t175*p2-p0;
	...
	const T t216 = p1*q3+t93*t175+q0*t192-t180*q2-t65+t185*q1;
	// partial u / partial q2
	jac_qt[0][1] = (p1*q1+t175*t65+t180*q0-t185*q3+t192*q2+t93)*t127-t147*t216;
	// partial v / partial q2
	jac_qt[1][1] = t206*t127-t159*t216;
	const T t240 = -q3/q0;
	const T t245 = t240*p0-p1;
	const T t250 = p1*t240+p0;
	const T t257 = t240*p2;
	...
	// partial u / partial x1
	jac_s[0][0] = (q1q1+q0q0-q3q3-q2q2)*t127-t147*t309;
	// partial v / partial x1
	jac_s[1][0] = t303*t127-t159*t309;
	const T t325 = q2q2+q0q0-q1q1-q3q3;
	const T t327 = q2*q3;
	const T t330 = -q0*q1;
	const T t331 = 2.0*(t327-t330);
	// partial u / partial x2
	jac_s[0][1] = 2.0*(t302+t299)*t127-t147*t331;
	// partial v / partial x2
	jac_s[1][1] = t325*t127-t159*t331;
	const T t347 = t327+2.0*t330+t327;
	const T t350 = q3q3+q0q0-q2q2-q1q1;
	// partial u / partial x1
	jac_s[0][2] = 2.0*(t305-t306)*t127-t147*t350;
	// partial v / partial x1
	jac_s[1][2] = t347*t127-t159*t350;
}
